...
static bstring DB_load() {
    ...
    check(data, "Failed to read from db: %s", DB_FILE);
...
int DB_update(const char *url) {
    ...
    check(rc==1, "Failed to append file to the db");
    return 0;
...
int DB_find(const char *url) {
    ...
    check(data, "Failed to load from file: %s", DB_FILE);
...
int DB_init() {
    ...
    if(access(DB_FILE, W_OK) == -1) {
        ...
        check(db,"Failed to create database: %s", DB_FILE);
        DB_close(db);
    }
